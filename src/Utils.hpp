...
public:
    Sprite() = default;
    Sprite(const std::string& texturePath, bool scrolling = false, float speed = 1.0f, int interval = 50, 
           const std::string& hoverTexturePath = "") 
        : sprite(cat(texturePath)), isScrolling(scrolling), 
          scrollSpeed(speed), updateInterval(interval) {
        if (!hoverTexturePath.empty()) {
            try {
                hoverSprite = cat(hoverTexturePath);
                isHoverable = true;
